...
func NewScheduler(ctx context.Context, logger *slog.Logger, leaser leases.Leaser) *Scheduler {
	return NewSchedulerForTesting(ctx, logger, leaser, time.Now)
}

// NewSchedulerForTesting creates a new [Scheduler] with mockable time.
func NewSchedulerForTesting(
	ctx context.Context,
	logger *slog.Logger,
	leaser leases.Leaser,
	now func() time.Time,
) *Scheduler {
	s := &Scheduler{logger: logger, now: now, leaser: leaser}
	go s.run(ctx)
	return s
}
